package com.dev.pizzaria.controller;

import java.io.Serializable;
import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.HttpStatus;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseStatus;
import org.springframework.web.bind.annotation.RestController;

import com.dev.pizzaria.model.Sabor;
import com.dev.pizzaria.repository.SaborRepository;

@RestController
@RequestMapping("/sabor")
public class SaborController implements Serializable{
	private static final long serialVersionUID = 1L;

	@Autowired SaborRepository saborRepository;
	
	@GetMapping("/all")
	public List<Sabor> all(){
		return saborRepository.findAll();
	}
	
	@PostMapping("/new")
	@ResponseStatus(code = HttpStatus.CREATED)
	public void newSabor(@RequestBody Sabor sabor) {
		saborRepository.save(sabor);
	}
}
